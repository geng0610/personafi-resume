...
(function () {
  ...
  if (!window.tableau._apiLoaded) {
    ...
    (function () {
      ...
      // Modified from jQuery's offset() implementation
      function elementOffset(element) {
        var rect, docElement;
        rect = element.getBoundingClientRect();
        docElement = document.documentElement;
        return {
          left: rect.left + windowPageXOffset() - docElement.clientLeft,
          top: rect.top + windowPageYOffset() - docElement.clientTop
        };
      }
      ...
      function urlFromObjectParams(viewName, serverRoot, params) {
        ...
        skipParams = {
          ...
          'site_prefix' : 1,
          'site_root' : 1,
          //BUGZID 161377 don't add :bootstrapWhenNotified so that viz doesn't spin infinitely
      ...
      function calculateServerRootFromObjectParameters(opts) {
        ...
        // BUGZID: 132568 - Only do the following replacement if the base window is not using the file protocol.
        if ((baseWindowProto.indexOf("file://") === -1) && (serverRoot.indexOf(baseWindowProto) === -1)) {
      ...
      function onMessage(e) {
        ...
        if (messageType === "tableau.completed" || messageType === "completed") {
          ...
          // $note-myork-2015-01-29: The viz also needs to know the current iframe offset from
          // the document, but the message doesn't contain which iframe we care about. Brute
          // force: send the viz offset for all vizs. B154185.
          for (i = 0; i <= loadIndex; i++) {
            viz = tableau.vizs[vizsLoadOrder[i]];
            if (viz) {
              viz._sendVizOffset();
          ...
          postLayoutInfo(e.source, tableau.vizs[vizsLoadOrder[loadIndex]]);
        }
      }
      ...
      LoadFeedback.prototype.createLoadingFeedback = function (serverRoot, objectParams) {
        ...
        html.push('       style="display:none; border: 0; cursor: wait; box-shadow: none;" />');
        html.push('</div>');
      ...
      window.tableau.Viz.prototype._sendVizOffset = function () {
        ...
        var pos, params, iframeOffset;
        iframeOffset = elementOffset(this._iframe);
        params = ['vizOffsetResp', iframeOffset.left, iframeOffset.top];
